...
* 染色体：一组不同基因的线性排列。**每条染色体上有多个不同的基因**，这些基因中既有完全独立的，也有相互关联的。**在遗传算法中，染色体一般用一个一维数组表示**。
...
![image-20191228212409159](\img\image-20191228212409159.png)
...
* 结束条件：一般用最大进化代数作为结束条件，也可以根据进化情况进行灵活判断。
...
每个个体都有一条长度为 $n+m-1$ 的染色体 `gene` 。染色体由 $1$ 到 $n$ 的全排列和 $m-1$ 个 $0$ 组成，这 $m-1$ 个 $0$ 将整个染色体分成了 $m$ 段，从左到右第 $i$ 段对应着第 $i$ 辆车的配送路线。**注意：可能有的 $0$ 是相邻的，或者位于数组边缘，这表示对应编号的车不参与配送**。
...
这种编码方案十分直观，能够很方便地解码成具体的方案，而且可以简化接下来的变异操作，提高程序的运行效率。
...
$$
其中 $score$ 为问题分析中介绍过的得分公式。取负是因为适应度越大越好，而得分越低越好。
...
1. 随机选定染色体数组中的两个元素 `gene[i]` 和 `gene[j]` 。
2. 交换两个元素的值。
3. 若变异后的个体不合法，则撤销变异操作。
...
> $$
> 其中 `gene[4] = 9` 和 `gene[6] = 7` 交换了位置，改变了第二辆车配送的相对顺序。
>
> 也可能产生如下新个体：
> $$
...
如果用传统的交叉算法（即随机交换两条染色体的片段）对染色体进行交叉操作，则交叉后产生的新染色体不一定是合法的，需要进行调整。由于调整过程十分复杂且消耗资源，为简单起见，这里**不对个体进行交叉操作**，仅仅通过变异操作产生新个体。

## 算法结束条件

程序会记录每次进化过程中个体的最大适应度。若最大适应度持续 $1000$ 代都无变化，则结束算法。
...
地图中的红点P表示配送中心，蓝点表示配送点，旁边的数字表示需求量。
